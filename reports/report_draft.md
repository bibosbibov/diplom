...
| **MAE по баллу CVSS** | **1.17** (11.7% шкалы) |
| RMSE по баллу CVSS | 1.98 |
...
|---|---:|---:|---:|
| AV | 0.5175 | 0.9023 | 972 |
| AC | 0.7482 | 0.9300 | 972 |
| AT | 0.7433 | 0.8837 | 972 |
| PR | 0.6308 | 0.7212 | 972 |
| UI | 0.6414 | 0.8827 | 972 |
| VC | 0.7886 | 0.7942 | 972 |
| VI | 0.8198 | 0.8241 | 972 |
| VA | 0.7946 | 0.7994 | 972 |
| SC | 0.6228 | 0.8704 | 972 |
| SI | 0.6705 | 0.8909 | 972 |
| SA | 0.6560 | 0.8920 | 972 |
| **E** | **0.8751** | **0.9512** | 553 |

Лидеры по F1 — импакт-метрики Vulnerable System (VC/VI/VA, 0.79–0.82) и Exploit Maturity (0.88). Аутсайдер — Attack Vector (0.52): малое количество классов Adjacent/Physical в обучающей выборке.

### 5.3. Стабильность val vs test

Максимальное отклонение F1 между лучшей эпохой валидации и тестом — `|Δ| ≤ 0.035` по всем 12 метрикам ([reports/final_results.md](reports/final_results.md#L36-L52)), что свидетельствует об отсутствии переобучения на val.
...
Выигрыш mBERT по F1 над сильнейшим baseline — `+0.0185` (≈ +1.85 п.п.); по Vector Accuracy лидирует RandomForest, что объясняется сбалансированной предвзятостью деревьев к мажоритарному классу.
...
| Macro-F1 | **0.800** | 0.764 |
| Vector accuracy | **0.501** | 0.476 |
| Метрик верно в среднем | **7.00 / 8 (87.5 %)** | 9.63 / 12 (80.3 %) |
| Severity accuracy | **0.723** | 0.713 |
| Severity ±1 уровень | **0.967** | 0.949 |
| Score MAE | **0.789** | 1.014 |
| Score RMSE | **1.368** | 1.857 |

Per-metric качество v3.1 — **развёрнутый режим** (через предиктор, текст без `cwe_name`; тест 26 317 записей, [reports/v31_results.md](reports/v31_results.md)):

| Метрика | Полное название | F1 (macro) | Accuracy |
|---|---|---:|---:|
| AV | Attack Vector | 0.7707 | 0.9333 |
| AC | Attack Complexity | 0.7097 | 0.9165 |
| PR | Privileges Required | 0.7502 | 0.8060 |
| UI | User Interaction | 0.8953 | 0.9084 |
| S | Scope | 0.8295 | 0.9049 |
| C | Confidentiality Impact | 0.8234 | 0.8369 |
| I | Integrity Impact | 0.8356 | 0.8409 |
| A | Availability Impact | 0.7854 | 0.8516 |
...
3. **Три разных значения F1 — не путать.** `0.850` — F1 **только Scope** (одна метрика, валидация); `0.809` — среднее по **7 головам** (тест, текст **с** `cwe_name`, `v3_dapt.json`); `0.800` — среднее по **8 метрикам** (тест, развёрнутый режим **без** `cwe_name`, `v31_results.md`). Реальные числа подтверждают механику: в развёрнутом режиме те же 7 голов дают в среднем `≈0.796` (против `0.809` с `cwe_name`) — отсутствие `cwe_name` в тексте предиктора стоит ≈1.3 п.п.; добавление Scope (на тесте `0.830`) поднимает среднее по 8 метрикам до `0.800`.

Это улучшение **реализовано**: оба предиктора (v4.0 и v3.1) теперь подставляют `cwe_name` по `cwe_id` через офлайн-справочник [src/data_preparation/cwe_names_lookup.py](src/data_preparation/cwe_names_lookup.py) (источник — `data/raw/cwe_names.json`, выгрузка MITRE). При отсутствии словаря лукап тихо возвращает `None`, и пайплайн работает как раньше. Ожидаемый эффект — возврат ≈+1 п.п. F1 на текстовых головах (и в v3.1, и в v4.0); точные числа подлежат подтверждению повторным прогоном `V31Evaluator`/`Evaluator`.
