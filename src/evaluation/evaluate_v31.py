...
:class:`~src.inference.VulnerabilityPredictorV31`: тот же текст без ``cwe_name``,
тот же расчёт балла через :class:`CVSS31Calculator`. Поэтому цифры отражают то,
что реально отдаёт API, и могут отличаться от ``v3_dapt.json`` (там per-head F1
считался по :class:`CVSSDataset` — с ``cwe_name`` в тексте).
